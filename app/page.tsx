...
function WeakBoundary({ line }: { line: string }) {
  // 之前的版本有 1px 实线 + 12px 渐变阴影，主题切换时形成清晰横线很硬�?  // 改为更柔和的 24px 渐变 + �?opacity，section 衔接更自然�?  return (
...
const FEATURES = [
  ...
  },
  {
    ...
    points: [
      ...
      '六大维度解读：事业方向、感情婚姻、财运模式、健康注意、家庭关系、子女缘�?,
      '大限流年追踪：当�?0年大限重点、今年流年宫位的具体提示与行动建�?,
  ...
  },
  {
    ...
    points: [
      '自动识别11种经典格局：紫府同宫、杀破狼格、机月同梁、廉相格、武曲七杀�?,
      '辅弼夹命、日月夹命等特殊格局精准检测，并给出倪海夏体系下的标准解�?,
      '四化入命宫迁移宫的特殊状况自动标注，提示需关注的人生议�?,
...
function useColors(theme: Theme) {
  ...
  return {
    ...
    // nav 用与 bgBase 完全相同的不透明色，避免半透明叠加产生色差�?    navBg:        d ? '#020810'                                : '#f5efe0',
    ...
    // 亮色文字用冷灰系（A 方案核心）：暖底 + 冷字 �?视觉不审美疲�?    textPrimary:  d ? '#e8eef6'                               : '#1a1d24',
    textSecond:   d ? '#b8c6df'                               : '#3a3f4a',
    textMuted:    d ? '#9db0d0'                               : '#5a6275',
    textFaint:    d ? 'rgba(240,246,255,0.56)'                : '#9da4b3',
    // 冷色 accent（B 方案核心）：呼应暗色 quan 蓝；用于装饰�?glow / 链接 / 高亮
...
const STAR_BRIEF: Record<string, { attr: string; brief: string }> = {
  ...
  '天同': { attr: '水·福�?, brief: '福德主星，主享乐与人缘。性情温和，人缘极好，注重生活品质，感情细腻，晚年运势佳�? },
  ...
  '天相': { attr: '水·印�?, brief: '印星，主辅佐与印绶。善于协调，重视礼节，正直守法，适合幕僚、行政、法律类工作，贵人运佳�? },
  '天梁': { attr: '土·荫�?, brief: '荫星，主老成与荫蔽。正直稳重，慈悲心强，老天爷会保佑，适合医疗、社会工作、宗教领域�? },
...
function FeatureVisual({ index, colors: c }: { index: number; colors: ReturnType<typeof useColors> }) {
  ...
  if (index === 1) {
    ...
    return (
      <div className="flex flex-col gap-4 h-full justify-center">
        {[
          { group: '紫微�?, stars: ['紫微', '天机', '太阳', '武曲', '天同', '廉贞'] },
          { group: '天府�?, stars: ['天府', '太阴', '贪狼', '巨门', '天相', '天梁', '七杀', '破军'] },
        ].map(group => (
          <div key={group.group}>
  ...
  if (index === 3) {
    const patterns = [
      { name: '杀破狼�?, desc: '开创进取之�?, ok: true },
      { name: '廉相�?,   desc: '行政印绶之格', ok: true },
...
export default function HomePage() {
  ...
  // �?body / html 背景同步�?home 主题色，消除半透明 nav 透出 #fafaf9 的色�?  // useLayoutEffect 保证在浏览器绘制前同步更新，避免与根 div �?transition 不同�?  useLayoutEffect(() => {
  ...
  return (
    <div style={{ background: c.bgBase, transition: 'background 0.35s ease' }} className="overflow-x-hidden">
      ...
      {/* ── 顶部导航 ── nav �?hero 同色（c.bgBase），�?blur �?border，彻底无色差�?*/}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-8 py-3 sm:py-4 gap-2"
        ...
        <div className="text-[11px] sm:text-xs tracking-[0.3em] sm:tracking-[0.4em] font-medium transition-colors duration-300 flex-shrink-0"
          style={{ color: c.goldSolid }}>紫气东源</div>
      ...
      <section ref={heroRef} className="relative min-h-[82svh] lg:min-h-[92vh] flex flex-col items-center justify-center px-6 z-10 pb-24 pt-10">
        <motion.div style={{ y: heroY, opacity: heroOpacity, maxWidth: '960px' }} className="text-center w-full mx-auto mt-10">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            输入出生年月日时，生成专属紫微斗数命�?�?后续天纪、地纪、人纪学习模块陆续开放�?          </motion.p>
        ...
        >
          <div style={{
            ...
            <div style={{ fontSize: '13px', lineHeight: 1.7, color: '#8b3a1a', fontWeight: 500 }}>
              全部功能 + AI 提问
              <strong style={{ color: '#c45a2d' }}> 全免�?/strong>
            </div>
          </div>
        </motion.div>

        {/* 上线公告便利�?�?手机端正常流式显示（hero 内容下方居中�?*/}
        <motion.div
        ...
        >
          <div style={{
            ...
            <div style={{ fontSize: '12px', lineHeight: 1.7, color: '#8b3a1a', fontWeight: 500 }}>
              全部功能 + AI <strong style={{ color: '#c45a2d' }}>全免�?/strong>
            </div>
          </div>
        </motion.div>

        {/* 滚动提示（绝对定位，不影�?hero opacity 计算�?*/}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <section className="relative z-10 overflow-hidden min-h-[82svh] lg:min-h-[92vh] flex items-center" style={{ padding: '72px 24px' }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden">
          <span className="font-bold" style={{ fontSize: 'clamp(220px, 38vw, 460px)', color: 'rgba(212,168,67,0.012)', lineHeight: 1, fontFamily: 'serif' }}>�?/span>
        </div>
        <FadeIn className="relative mx-auto text-center w-full" y={20}>
          <div className="flex items-center justify-center gap-4 mb-8">
            ...
            <span className="text-[10px] tracking-[0.55em] uppercase" style={{ color: 'rgba(212,168,67,0.5)' }}>�?· �?· �?/span>
          ...
          <div className="space-y-3" style={{ maxWidth: '840px', margin: '0 auto' }}>
            {[
              { text: '提前窥探命运的意�?, size: 'clamp(17px, 2.2vw, 28px)', color: 'rgba(215,228,252,0.72)', delay: 0.1 },
              { text: '不在于预知未�?, size: 'clamp(21px, 2.6vw, 32px)', color: 'rgba(220,232,250,0.74)', delay: 0.25 },
              { text: '而在于不断认识自�?, size: 'clamp(24px, 3vw, 40px)', color: 'rgba(218,230,248,0.8)', delay: 0.34 },
            ].map((line, i) => (
              <motion.p key={i}
              ...
              return (
                <motion.div key={s.key}
                  ...
                  <div className="flex-1 lg:flex-none flex flex-col items-start lg:items-center min-w-0">
                    {/* 顶行：时间标�?+ 板块�?+ note（手机端 inline；桌面端依然分行�?*/}
          ...
          <FadeIn>
            <div className="text-center mb-10">
              ...
              <p className="max-w-2xl mx-auto text-sm leading-relaxed" style={{ color: c.textSecond }}>
                ...
                他将影响人生的力量分为三个同等重要的维度�?              </p>
            </div>
          </FadeIn>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
            {[
              { glyph: '�?, label: '先天命运', pct: '�?, color: c.goldSolid, borderColor: c.goldLine, desc: '紫微斗数所揭示的，是一个人的先天命盘格局——出生时间决定的星曜布局、五行局数、命宫主星。这只是命运的三分之一，是人生的底色，而非全貌�?, sub: '命盘 · 星曜 · 五行' },
              { glyph: '�?, label: '地理环境', pct: '�?, color: 'rgba(96,165,250,0.9)', borderColor: 'rgba(96,165,250,0.3)', desc: '你所在的地理环境、城市、国家、风水格局，乃至家庭背景与社会结构，共同构成了命运的第二个维度。同一命盘，生在不同地方，际遇可以天壤之别�?, sub: '地域 · 风水 · 环境' },
              { glyph: '�?, label: '人心意念', pct: '�?, color: 'rgba(100,216,139,0.9)', borderColor: 'rgba(100,216,139,0.3)', desc: '个人的意志、心态、选择与行动，才是改变命运最主动的力量。倪师强调：了解命盘是为了更好地做人，而不是坐等命运安排。精进自己，是最强的破局之道�?, sub: '意志 · 选择 · 行动' },
            ].map((item, i) => (
          ...
          <FadeIn delay={0.3}>
            <div className="mt-10 text-center">
              <p className="text-sm leading-relaxed" style={{ color: c.textSecond }}>
                「命运不是人生的全部，加上地理位置和人念，才是。�?              </p>
              <p className="mt-2 text-[10px] tracking-widest" style={{ color: c.tagText }}>�?倪海�?/p>
            </div>
          </FadeIn>
        </div>
      </section>
          ...
          <FadeIn delay={0.1}>
            <div className="rounded-2xl p-8 md:p-10 mb-8"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                {[
                  { label: '生于', value: '1954�?, sub: '台湾' },
                  { label: '离世', value: '2012�?, sub: '1�?1�?· 享年58' },
              ...
              <div className="space-y-4 text-sm leading-relaxed max-w-3xl mx-auto" style={{ color: c.textSecond }}>
                <p>
                  <strong style={{ color: c.goldSolid }}>生平履历</strong>�?                  倪海夏先生（1954�?012）出生于台湾，早年师承多位中医名家，专研经方派（《伤寒论》传承）�?                  中年赴美行医，在美国创立<strong>汉唐中医学院</strong>，二十余年间系统传授中医与传统术数�?                  2012 �?1 �?31 日因肝癌在台湾离世，享年 58 岁�?                </p>
                <p>
                  <strong style={{ color: c.goldSolid }}>教学体系</strong>�?                  倪师将毕生所学整理为两大公开教学系列�?                  <strong>「人纪�?/strong>涵盖《针灸大成》《神农本草经》《黄帝内经》《伤寒论》《金匮要略》—�?                  这是「人之纪」，奠定中医学习的完整路径；
                  <strong>「天纪�?/strong>涵盖紫微斗数与《易经》——这是「天之纪」，是术数研究的体系化成果�?                  两者相合，是倪师留给后世最完整的传承�?                </p>
                <p>
                  <strong style={{ color: c.goldSolid }}>紫微立场</strong>�?                  倪师在紫微斗数上明确�?strong>南派三合�?/strong>，主张「以命宫为本、以三方四正为用、以四化为纲」�?                  他在《天纪》课程中明言：�?em>飞星（四化）飞来飞去太复杂，不搞这个，毕竟大道至简</em>」—�?                  这一立场将其与繁琐的飞星派清晰区分�?                </p>
                <p>
                  <strong style={{ color: c.goldSolid }}>治学态度</strong>�?                  倪师反对死记硬背口诀，强调「理解原理胜过背诵」「逻辑可复核胜过神秘玄学」�?                  这种态度让紫微斗数从师徒密传的封闭体系，走向系统化、可验证、可学习的现代知识体系�?                </p>
                <p>
                  <strong style={{ color: c.goldSolid }}>当代影响</strong>�?                  倪师的讲课视频在 B 站、YouTube 与各大平台广泛流传，是新一代命理与中医爱好者公认的入门必修�?                  他不仅是紫微斗数的传承者，更是把传统命理与中医带入现代知识体系的关键人物之一�?                </p>
                <p style={{ fontSize: '11px', color: c.textMuted, fontStyle: 'italic', marginTop: '12px' }}>
                  本平台所有解读基于倪师《天纪》公开教学讲义、《紫微斗数全书》明版、传统三合派古籍整理而成�?                  仅作文化与个人成长参考。倪师本人与本平台无任何商业关联�?                </p>
              </div>
            </div>
          </FadeIn>
          ...
          <div className="rounded-2xl p-10 md:p-14 text-center"
            ...
            <FadeIn>
              ...
              <p className="text-sm leading-relaxed mb-8 max-w-lg mx-auto" style={{ color: c.textSecond }}>
                ...
                给出感情匹配度、合伙可行性与最佳相处建议�?              </p>
              <div className="flex justify-center gap-3 flex-wrap mb-6">
                {['感情匹配度分�?, '合伙创业评估', '亲子缘分解读', '婚前相性评�?].map(item => (
                  <span key={item} style={{
      ...
      <section className="relative z-10 py-40 px-6 text-center" style={{ background: c.altSection }}>
        <FadeIn>
          <p className="text-[10px] tracking-[0.6em] uppercase mb-6" style={{ color: c.tagText }}>开始你的命盘之�?/p>
